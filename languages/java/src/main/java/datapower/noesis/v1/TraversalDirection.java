// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: datapower/noesis/v1/connector.proto

package datapower.noesis.v1;

/**
 * Protobuf enum {@code datapower.noesis.v1.TraversalDirection}
 */
public enum TraversalDirection
    implements com.google.protobuf.ProtocolMessageEnum {
  /**
   * <code>TRAV_DIR_UNSPECIFIED = 0;</code>
   */
  TRAV_DIR_UNSPECIFIED(0),
  /**
   * <code>OUTBOUND = 1;</code>
   */
  OUTBOUND(1),
  /**
   * <code>INBOUND = 2;</code>
   */
  INBOUND(2),
  /**
   * <code>BOTH = 3;</code>
   */
  BOTH(3),
  UNRECOGNIZED(-1),
  ;

  /**
   * <code>TRAV_DIR_UNSPECIFIED = 0;</code>
   */
  public static final int TRAV_DIR_UNSPECIFIED_VALUE = 0;
  /**
   * <code>OUTBOUND = 1;</code>
   */
  public static final int OUTBOUND_VALUE = 1;
  /**
   * <code>INBOUND = 2;</code>
   */
  public static final int INBOUND_VALUE = 2;
  /**
   * <code>BOTH = 3;</code>
   */
  public static final int BOTH_VALUE = 3;


  public final int getNumber() {
    if (this == UNRECOGNIZED) {
      throw new java.lang.IllegalArgumentException(
          "Can't get the number of an unknown enum value.");
    }
    return value;
  }

  /**
   * @param value The numeric wire value of the corresponding enum entry.
   * @return The enum associated with the given numeric wire value.
   * @deprecated Use {@link #forNumber(int)} instead.
   */
  @java.lang.Deprecated
  public static TraversalDirection valueOf(int value) {
    return forNumber(value);
  }

  /**
   * @param value The numeric wire value of the corresponding enum entry.
   * @return The enum associated with the given numeric wire value.
   */
  public static TraversalDirection forNumber(int value) {
    switch (value) {
      case 0: return TRAV_DIR_UNSPECIFIED;
      case 1: return OUTBOUND;
      case 2: return INBOUND;
      case 3: return BOTH;
      default: return null;
    }
  }

  public static com.google.protobuf.Internal.EnumLiteMap<TraversalDirection>
      internalGetValueMap() {
    return internalValueMap;
  }
  private static final com.google.protobuf.Internal.EnumLiteMap<
      TraversalDirection> internalValueMap =
        new com.google.protobuf.Internal.EnumLiteMap<TraversalDirection>() {
          public TraversalDirection findValueByNumber(int number) {
            return TraversalDirection.forNumber(number);
          }
        };

  public final com.google.protobuf.Descriptors.EnumValueDescriptor
      getValueDescriptor() {
    if (this == UNRECOGNIZED) {
      throw new java.lang.IllegalStateException(
          "Can't get the descriptor of an unrecognized enum value.");
    }
    return getDescriptor().getValues().get(ordinal());
  }
  public final com.google.protobuf.Descriptors.EnumDescriptor
      getDescriptorForType() {
    return getDescriptor();
  }
  public static final com.google.protobuf.Descriptors.EnumDescriptor
      getDescriptor() {
    return datapower.noesis.v1.ConnectorOuterClass.getDescriptor().getEnumTypes().get(7);
  }

  private static final TraversalDirection[] VALUES = values();

  public static TraversalDirection valueOf(
      com.google.protobuf.Descriptors.EnumValueDescriptor desc) {
    if (desc.getType() != getDescriptor()) {
      throw new java.lang.IllegalArgumentException(
        "EnumValueDescriptor is not for this type.");
    }
    if (desc.getIndex() == -1) {
      return UNRECOGNIZED;
    }
    return VALUES[desc.getIndex()];
  }

  private final int value;

  private TraversalDirection(int value) {
    this.value = value;
  }

  // @@protoc_insertion_point(enum_scope:datapower.noesis.v1.TraversalDirection)
}

